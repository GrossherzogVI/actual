 ...
 *
 * DKB exports start with several metadata rows before the actual header row.
 * We look for a row that contains "Buchungstag" or "Buchungsdatum" to find
 * the header, then parse all subsequent rows.
 *
 * Expected columns (after header detection):
 ...
 */
export function parseDkb(text: string): ParserResult {
  ...
  if (headerIndex === -1) {
    return {
      rows: [],
      errors: ['DKB-Format nicht erkannt: Kein Buchungstag-Header gefunden.'],
      bankName: 'DKB',
  ...
  const amountCol = colIndex(['Betrag (EUR)', 'Betrag']);
  const payeeCol = colIndex(['Auftraggeber / Beg\u00fcnstigter', 'Auftraggeber', 'Begünstigter']);
  const notesCol = colIndex(['Verwendungszweck']);
  const ibanCol = colIndex(['Kontonummer', 'IBAN']);

  if (dateCol === -1 || amountCol === -1) {
    return {
      rows: [],
      errors: ['DKB-Format: Pflichtfelder (Datum, Betrag) nicht gefunden.'],
      bankName: 'DKB',
      encoding: 'windows-1252',
    };
  }
  ...
  return { rows, errors, bankName: 'DKB', encoding: 'windows-1252' };
}
