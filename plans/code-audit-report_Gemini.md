...
- **packages/loot-core/src/shared/deadlines.ts:92** — `addBusinessDays` contains an unoptimized logic flaw. The `while (remaining > 0)` loop consecutively invokes `isBusinessDay(d, bundesland)`, which recalculates `getHolidays()` on _every single iteration_. Impact: O(N) performance hit rebuilding an entire Set of 16 strings per iteration step. Fix: Memoize `getHolidays()` or pre-extract the Set before the loop.
