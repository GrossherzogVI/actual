...
app.post('/presets', (req, res) => {
  ...
  const created = db.first('SELECT * FROM quick_add_presets WHERE id = ?', [
    id,
  ]);
...
app.patch('/presets/:id', (req, res) => {
  const db = getAccountDb();
  const existing = db.first('SELECT id FROM quick_add_presets WHERE id = ?', [
    req.params.id,
  ]);
  ...
  const updated = db.first('SELECT * FROM quick_add_presets WHERE id = ?', [
    req.params.id,
  ]);
